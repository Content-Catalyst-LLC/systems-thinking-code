...
fig <- file.path(root, "outputs", "figures", "policy_outcome_trajectories.png")
png(fig, width = 1000, height = 650)
scenarios <- unique(policy_results$scenario)
plot(NULL, xlim = range(policy_results$year), ylim = c(0, 100), xlab = "Year", ylab = "Policy outcome index", main = "Policy Outcome Trajectories by Scenario")
for (i in seq_along(scenarios)) {
  x <- policy_results[policy_results$scenario == scenarios[i], ]
  lines(x$year, x$policy_outcome, lwd = 2, lty = i)
}
