#!/usr/bin/env Rscript
# Professional R diagnostics for: Stock Flow Thinking in Social Systems
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("Stock Flow Thinking in Social Systems", "—", label))
...
message("Professional R diagnostics complete for: Stock Flow Thinking in Social Systems")
