#!/usr/bin/env Rscript
# Professional R diagnostics for: What Is Systems Thinking
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("What Is Systems Thinking", "—", label))
...
message("Professional R diagnostics complete for: What Is Systems Thinking")
