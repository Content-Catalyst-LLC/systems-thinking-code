#!/usr/bin/env Rscript
# Professional R diagnostics for: Dynamic Complexity and Policy Resistance
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("Dynamic Complexity and Policy Resistance", "—", label))
...
message("Professional R diagnostics complete for: Dynamic Complexity and Policy Resistance")
