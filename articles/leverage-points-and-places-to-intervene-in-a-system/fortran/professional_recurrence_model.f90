! Professional recurrence model for Leverage Points and Places to Intervene in a System
program professional_recurrence_model
  implicit none
  integer :: year
  real :: stock, capacity, burden, pressure, risk
  stock = 80; capacity = 49; burden = 42
  print *, 'year,stock,capacity,burden,risk'
  do year = 0, 30
     pressure = 4.7 + 2.1 * real(year) * 0.45
     burden = max(0.0, min(100.0, burden + pressure * 0.08 - 1.2 * 0.35))
     capacity = max(0.0, min(100.0, capacity + 1.2 - burden * 0.015))
     stock = max(0.0, min(120.0, stock + stock * 0.025 - pressure - burden * 0.035 + capacity * 0.025))
