// Professional threshold scan for Leverage Points and Places to Intervene in a System
#include <algorithm>
#include <iomanip>
#include <iostream>
...
int main() {
    ...
    for (double pr = 0.0; pr <= 0.5001; pr += 0.1) { for (double ci = 0.0; ci <= 3.0001; ci += 0.5) {
        double stock = 80, capacity = 49, burden = 42;
        for (int year = 0; year <= 30; ++year) {
            double pressure = (4.7 + 2.1 * year * 0.45) * (1.0 - pr);
            burden = clamp(burden + pressure * 0.08 - ci * 0.35, 0.0, 100.0);
            capacity = clamp(capacity + ci - burden * 0.015, 0.0, 100.0);
            stock = clamp(stock + stock * 0.025 - pressure - burden * 0.035 + capacity * 0.025, 0.0, 120.0);
        }
