#!/usr/bin/env Rscript
# Professional R diagnostics for: Leverage Points and Places to Intervene in a System
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("Leverage Points and Places to Intervene in a System", "—", label))
...
message("Professional R diagnostics complete for: Leverage Points and Places to Intervene in a System")
