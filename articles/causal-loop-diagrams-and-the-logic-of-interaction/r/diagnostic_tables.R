#!/usr/bin/env Rscript
# Professional R diagnostics for: Causal Loop Diagrams and the Logic of Interaction
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("Causal Loop Diagrams and the Logic of Interaction", "—", label))
...
message("Professional R diagnostics complete for: Causal Loop Diagrams and the Logic of Interaction")
