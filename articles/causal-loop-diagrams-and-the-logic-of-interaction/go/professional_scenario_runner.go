...
func main() {
    ...
    for pr := 0.0; pr <= 0.5001; pr += 0.10 { for ci := 0.0; ci <= 3.0001; ci += 0.50 {
        stock := float64(82); capacity := float64(51); burden := float64(34)
        for year := 0; year <= 30; year++ {
            pressure := (4.3 + 2.0*float64(year)*0.45) * (1.0 - pr)
            burden = clamp(burden + pressure*0.08 - ci*0.35, 0, 100)
            capacity = clamp(capacity + ci - burden*0.015, 0, 100)
            stock = clamp(stock + stock*0.027 - pressure - burden*0.035 + capacity*0.025, 0, 120)
        }
        risk := clamp(100 - (0.45*stock + 0.35*capacity - 0.20*burden), 0, 100)
        _ = w.Write([]string{"causal-loop-diagrams-and-the-logic-of-interaction", fmt.Sprintf("%.2f", pr), fmt.Sprintf("%.2f", ci), fmt.Sprintf("%.2f", stock), fmt.Sprintf("%.2f", risk)})
    } }
