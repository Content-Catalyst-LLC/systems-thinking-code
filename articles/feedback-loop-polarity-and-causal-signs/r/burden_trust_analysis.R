#!/usr/bin/env Rscript
# Professional R diagnostics for: Feedback Loop Polarity and Causal Signs
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("Feedback Loop Polarity and Causal Signs", "—", label))
...
message("Professional R diagnostics complete for: Feedback Loop Polarity and Causal Signs")
