#!/usr/bin/env Rscript
# Professional R diagnostics for: System Dynamics and Simulation Modeling
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("System Dynamics and Simulation Modeling", "—", label))
...
message("Professional R diagnostics complete for: System Dynamics and Simulation Modeling")
