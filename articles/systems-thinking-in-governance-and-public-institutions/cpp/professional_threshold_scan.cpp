// Professional threshold scan for Systems Thinking in Governance and Public Institutions
#include <algorithm>
#include <iomanip>
#include <iostream>
...
int main() {
    ...
    for (double pr = 0.0; pr <= 0.5001; pr += 0.1) { for (double ci = 0.0; ci <= 3.0001; ci += 0.5) {
        double stock = 76, capacity = 48, burden = 48;
        for (int year = 0; year <= 30; ++year) {
            double pressure = (4.1 + 2.0 * year * 0.45) * (1.0 - pr);
