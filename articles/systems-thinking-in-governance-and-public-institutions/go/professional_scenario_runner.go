...
func main() {
    ...
    for pr := 0.0; pr <= 0.5001; pr += 0.10 { for ci := 0.0; ci <= 3.0001; ci += 0.50 {
        stock := float64(76); capacity := float64(48); burden := float64(48)
        for year := 0; year <= 30; year++ {
            pressure := (4.1 + 2.0*float64(year)*0.45) * (1.0 - pr)
        ...
        _ = w.Write([]string{"systems-thinking-in-governance-and-public-institutions", fmt.Sprintf("%.2f", pr), fmt.Sprintf("%.2f", ci), fmt.Sprintf("%.2f", stock), fmt.Sprintf("%.2f", risk)})
    } }
