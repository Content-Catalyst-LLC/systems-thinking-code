#!/usr/bin/env julia
# Professional dynamic sensitivity model for Systems Thinking in Governance and Public Institutions
function clamp01(x, lo=0.0, hi=100.0)
    return max(lo, min(hi, x))
end
base_stock = 76; base_capacity = 48; base_burden = 48
regen = 0.026; degrade = 4.1
function simulate(pressure_reduction, capacity_investment; years=30)
    ...
    for year in 0:years
        pressure = (degrade + 2.0 * year * 0.45) * (1.0 - pressure_reduction)
