#!/usr/bin/env Rscript
# Professional R diagnostics for: Tragedy of the Commons and Shared Resource Systems
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("Tragedy of the Commons and Shared Resource Systems", "—", label))
...
message("Professional R diagnostics complete for: Tragedy of the Commons and Shared Resource Systems")
