#!/usr/bin/env Rscript
# Professional R diagnostics for: Feedback Loops and System Behavior
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("Feedback Loops and System Behavior", "—", label))
...
message("Professional R diagnostics complete for: Feedback Loops and System Behavior")
