#!/usr/bin/env Rscript
# Professional R diagnostics for: System Boundaries and Problem Framing
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("System Boundaries and Problem Framing", "—", label))
...
message("Professional R diagnostics complete for: System Boundaries and Problem Framing")
