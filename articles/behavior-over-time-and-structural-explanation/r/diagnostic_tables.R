#!/usr/bin/env Rscript
# Professional R diagnostics for: Behavior Over Time and Structural Explanation
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("Behavior Over Time and Structural Explanation", "—", label))
...
message("Professional R diagnostics complete for: Behavior Over Time and Structural Explanation")
