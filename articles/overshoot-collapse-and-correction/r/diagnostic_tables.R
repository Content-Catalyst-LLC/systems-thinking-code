#!/usr/bin/env Rscript
# Professional R diagnostics for: Overshoot Collapse and Correction
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("Overshoot Collapse and Correction", "—", label))
...
message("Professional R diagnostics complete for: Overshoot Collapse and Correction")
