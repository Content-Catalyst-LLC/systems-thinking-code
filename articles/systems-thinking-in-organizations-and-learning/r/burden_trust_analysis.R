#!/usr/bin/env Rscript
# Professional R diagnostics for: Systems Thinking in Organizations and Learning
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("Systems Thinking in Organizations and Learning", "—", label))
...
message("Professional R diagnostics complete for: Systems Thinking in Organizations and Learning")
