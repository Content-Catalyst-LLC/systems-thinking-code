#!/usr/bin/env Rscript
# Professional R diagnostics for: Success to the Successful and Systemic Advantage
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("Success to the Successful and Systemic Advantage", "—", label))
...
message("Professional R diagnostics complete for: Success to the Successful and Systemic Advantage")
