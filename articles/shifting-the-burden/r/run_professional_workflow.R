#!/usr/bin/env Rscript
# Professional R diagnostics for: Shifting the Burden
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("Shifting the Burden", "—", label))
...
message("Professional R diagnostics complete for: Shifting the Burden")
