#!/usr/bin/env Rscript
# Professional R diagnostics for: Sensitivity Analysis For System Interventions
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("Sensitivity Analysis For System Interventions", "—", label))
...
message("Professional R diagnostics complete for: Sensitivity Analysis For System Interventions")
