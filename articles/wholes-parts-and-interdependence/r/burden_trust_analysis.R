#!/usr/bin/env Rscript
# Professional R diagnostics for: Wholes Parts and Interdependence
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("Wholes Parts and Interdependence", "—", label))
...
message("Professional R diagnostics complete for: Wholes Parts and Interdependence")
