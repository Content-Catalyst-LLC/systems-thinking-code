#!/usr/bin/env Rscript
# Professional R diagnostics for: Delayed Feedback and Policy Timing
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("Delayed Feedback and Policy Timing", "—", label))
...
message("Professional R diagnostics complete for: Delayed Feedback and Policy Timing")
