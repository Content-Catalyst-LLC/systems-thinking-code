#!/usr/bin/env Rscript
# Professional R diagnostics for: Mental Models and the Limits of Linear Reasoning
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("Mental Models and the Limits of Linear Reasoning", "—", label))
...
message("Professional R diagnostics complete for: Mental Models and the Limits of Linear Reasoning")
