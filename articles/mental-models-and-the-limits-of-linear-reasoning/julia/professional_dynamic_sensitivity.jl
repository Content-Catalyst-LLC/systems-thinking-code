#!/usr/bin/env julia
# Professional dynamic sensitivity model for Mental Models and the Limits of Linear Reasoning
function clamp01(x, lo=0.0, hi=100.0)
    return max(lo, min(hi, x))
end
base_stock = 74; base_capacity = 46; base_burden = 52
regen = 0.024; degrade = 5.0
function simulate(pressure_reduction, capacity_investment; years=30)
    ...
    for year in 0:years
        pressure = (degrade + 2.4 * year * 0.45) * (1.0 - pressure_reduction)
