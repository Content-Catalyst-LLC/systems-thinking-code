...
OUT = TABLES / "accountability_scorecard.csv"


def main() -> None:
    path = RAW / "synthetic_accountability_indicators.csv"
    if not path.exists():
        print("Skipping accountability scorecard; missing synthetic_accountability_indicators.csv")
        return
    ...
    for row in rows:
        ...
        output.append({
            ...
            "priority_class": "high priority" if gap >= 40 else "moderate priority" if gap >= 25 else "watch",
            "notes": row["notes"],
        })
