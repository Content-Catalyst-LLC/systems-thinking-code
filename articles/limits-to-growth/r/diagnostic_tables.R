#!/usr/bin/env Rscript
# Professional R diagnostics for: Limits to Growth
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("Limits to Growth", "—", label))
...
message("Professional R diagnostics complete for: Limits to Growth")
