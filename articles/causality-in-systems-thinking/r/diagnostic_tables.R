#!/usr/bin/env Rscript
# Professional R diagnostics for: Causality in Systems Thinking
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("Causality in Systems Thinking", "—", label))
...
message("Professional R diagnostics complete for: Causality in Systems Thinking")
