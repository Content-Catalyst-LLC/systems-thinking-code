#!/usr/bin/env Rscript
# Professional R diagnostics for: Stocks Flows and the Architecture of Change
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("Stocks Flows and the Architecture of Change", "—", label))
...
message("Professional R diagnostics complete for: Stocks Flows and the Architecture of Change")
