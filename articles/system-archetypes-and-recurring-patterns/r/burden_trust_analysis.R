#!/usr/bin/env Rscript
# Professional R diagnostics for: System Archetypes and Recurring Patterns
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("System Archetypes and Recurring Patterns", "—", label))
...
message("Professional R diagnostics complete for: System Archetypes and Recurring Patterns")
