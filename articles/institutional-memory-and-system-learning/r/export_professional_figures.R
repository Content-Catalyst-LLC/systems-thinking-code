#!/usr/bin/env Rscript
# Professional R diagnostics for: Institutional Memory and System Learning
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("Institutional Memory and System Learning", "—", label))
...
message("Professional R diagnostics complete for: Institutional Memory and System Learning")
