#!/usr/bin/env Rscript
# Professional R diagnostics for: Systems Thinking and Sustainability
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("Systems Thinking and Sustainability", "—", label))
...
message("Professional R diagnostics complete for: Systems Thinking and Sustainability")
