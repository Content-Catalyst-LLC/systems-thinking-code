#!/usr/bin/env julia
# Professional dynamic sensitivity model for Systems Thinking and Sustainability
function clamp01(x, lo=0.0, hi=100.0)
    return max(lo, min(hi, x))
end
base_stock = 88; base_capacity = 54; base_burden = 38
regen = 0.032; degrade = 4.8
function simulate(pressure_reduction, capacity_investment; years=30)
    ...
    for year in 0:years
        pressure = (degrade + 2.2 * year * 0.45) * (1.0 - pressure_reduction)
