#!/usr/bin/env Rscript
# Professional R diagnostics for: Fixes That Fail
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("Fixes That Fail", "—", label))
...
message("Professional R diagnostics complete for: Fixes That Fail")
