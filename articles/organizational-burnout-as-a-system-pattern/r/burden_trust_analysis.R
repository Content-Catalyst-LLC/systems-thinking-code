#!/usr/bin/env Rscript
# Professional R diagnostics for: Organizational Burnout as a System Pattern
...
plot_series <- function(column, label, filename, legend_position) {
  ...
  plot(NA, xlim = range(df$year), ylim = range(df[[column]], na.rm = TRUE), xlab = "Year", ylab = label, main = paste("Organizational Burnout as a System Pattern", "—", label))
...
message("Professional R diagnostics complete for: Organizational Burnout as a System Pattern")
