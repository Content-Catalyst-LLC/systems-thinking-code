#!/usr/bin/env julia
# Professional dynamic sensitivity model for Systems Thinking Patterns Interdependence Structural Change
function clamp01(x, lo=0.0, hi=100.0)
    return max(lo, min(hi, x))
end
base_stock = 80; base_capacity = 50; base_burden = 36
regen = 0.026; degrade = 4.2
function simulate(pressure_reduction, capacity_investment; years=30)
    ...
    for year in 0:years
        pressure = (degrade + 1.9 * year * 0.45) * (1.0 - pressure_reduction)
