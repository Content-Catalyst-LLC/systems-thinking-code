# Professional Workflow Upgrade Manifest

This repository has been upgraded so each existing article directory contains runnable professional systems-analysis workflows.

## Upgraded article directories

- `articles/behavior-over-time-and-structural-explanation/`
- `articles/causal-loop-diagrams-and-the-logic-of-interaction/`
- `articles/causality-in-systems-thinking/`
- `articles/delayed-feedback-and-policy-timing/`
- `articles/delays-oscillation-and-misperception/`
- `articles/dynamic-complexity-and-policy-resistance/`
- `articles/feedback-loop-polarity-and-causal-signs/`
- `articles/feedback-loops-and-system-behavior/`
- `articles/fixes-that-fail/`
- `articles/institutional-memory-and-system-learning/`
- `articles/learning-organizations-and-feedback-awareness/`
- `articles/leverage-points-and-places-to-intervene-in-a-system/`
- `articles/limits-to-growth/`
- `articles/mental-models-and-the-limits-of-linear-reasoning/`
- `articles/organizational-burnout-as-a-system-pattern/`
- `articles/overshoot-collapse-and-correction/`
- `articles/paradigms-goals-and-deep-system-change/`
- `articles/patterns-events-and-structural-explanation/`
- `articles/policy-resistance-and-structural-redesign/`
- `articles/reinforcing-and-balancing-dynamics/`
- `articles/scenario-modeling-in-systems-thinking/`
- `articles/sensitivity-analysis-for-system-interventions/`
- `articles/shifting-the-burden/`
- `articles/stock-flow-thinking-in-social-systems/`
- `articles/stocks-flows-and-the-architecture-of-change/`
- `articles/success-to-the-successful-and-systemic-advantage/`
- `articles/system-archetypes-and-recurring-patterns/`
- `articles/system-boundaries-and-problem-framing/`
- `articles/system-dynamics-and-simulation-modeling/`
- `articles/systems-thinking-and-levels-of-analysis/`
- `articles/systems-thinking-and-sustainability/`
- `articles/systems-thinking-in-governance-and-public-institutions/`
- `articles/systems-thinking-in-organizations-and-learning/`
- `articles/systems-thinking-in-public-policy/`
- `articles/systems-thinking-patterns-interdependence-structural-change/`
- `articles/tragedy-of-the-commons-and-shared-resource-systems/`
- `articles/what-is-systems-thinking/`
- `articles/wholes-parts-and-interdependence/`

## Run all workflows

```bash
python3 scripts/run_all_professional_systems_workflows.py
Rscript scripts/run_all_professional_systems_workflows.R
```

The R runner is optional and requires `Rscript` to be installed.
