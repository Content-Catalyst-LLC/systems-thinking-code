...
and exits successfully so the default repository smoke test remains robust.
"""
...
def dependency_available(name: str) -> bool:
    """Return True when an optional Python dependency is importable."""
...
def main() -> int:
    ...
    for script in ADVANCED_SCRIPTS:
        ...
        article_dir = script.parents[1]
        print(f"\n=== Running {relative} ===")
        result = subprocess.run([sys.executable, str(script)], cwd=str(article_dir))
